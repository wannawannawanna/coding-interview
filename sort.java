// 堆排序:
//   堆排序是利用堆这种数据结构而设计的一种排序算法，堆排序是一种选择排序，它的最坏，最好，平均时间复杂度均为O(nlogn)。空间复杂度o(1)，不稳定排序
// 堆:
...
public class HeapSort {
    ...
    public static void adjustHeap(int []arr,int i,int length){
        ...
        for(int k=i*2+1;k<length;k=k*2+1){//从i结点的左子结点开始，也就是2i+1处开始
            ...
            if(arr[k] >temp){//如果子节点大于父节点，将子节点值赋给父节点（不用进行交换），跟temp比较，保证每一个子树都是大顶堆，
		    //每一个子树里面最大值会网上走,因为从下往上调整堆，所以每次看一个子树的时候他的最大值能往上走的同时也能保证每一个子树内部是大顶堆
                arr[i] = arr[k];
                i = k;
            }else{
                break;
...
//快速排序，也适用于链表的写法，partition的时间复杂度o(n),QuickSort的时间复杂度是o(logn),快速排序的时间复杂度即o(nlogn)。空间复杂度o(1),无论数组还是链表都是o(1)，不稳定排序
...
//顺序读取，合并到一个文件中
// 注意, 首先要把数组分成两部分, 然后再把两部分当做有序数组来进行合并,时间复杂度o(NlogN)，数组排序的话，空间复杂度是o(N)；但是链表排序的话空间复杂度o(1)，稳定排序
import java.util.Arrays;
...
//   平均插入排序算法的时间复杂度为 O(n²)。因而，插入排序不适合对于数据量比较大的排序应用。稳定排序
...
// 算法的核心在于每次通过两两比较交换位置，选出剩余无序序列里最大（小）的数据元素放到队尾。时间复杂度o(n^2),空间复杂度o(1),稳定排序
...
/*
选择排序，依次选出最大（小）的元素，并将他的位置和尚未排序的数组中最后一个元素互换，直到完成所有的元素的选择，借助一个max（min）和index，时间复杂度o(n^2),空间复杂度o(1).稳定排序
*/
package bishi;
...
会越来越多，当步长为1的时候，整个数据合成一组，构成一组有序记录，完成排序。时间复杂度o(nlogn) ~ o(n2),空间复杂度o(1)，不稳定排序
*/
package bishi;
